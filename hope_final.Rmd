...
drought_severity$year <- format(drought_severity$releaseDate, format="%y")
...
tidy_drought_coord <- drought_coord%>%
  gather(key = category, vaue = value, NONE, D0, D1, D2, D3, D4)
```
...
#all aprils over each year (2000 and 2010-2015)
tidy_coord_aprils <- tidy_drought_coord%>%
  filter(year == "00" | year == "10" | year == "11" | year == "12" | year == "13" | year == "14" | year == "15")%>%
  group_by(month, year, subregion, category, long, lat, group, order)%>%
  summarise(ave = mean(value))%>%
  filter(month == "April")

tidy_coord_aprils <- data.frame(tidy_coord_aprils)


#trying to plot here, testing different data and map info needed

basemap <- ggplot(CAcounties, aes(x = long, y = lat, group = group)) 

basemap +
  coord_equal() + 
  geom_polygon(data = (tidy_coord_aprils), aes(fill = as.factor(ave))) +
  scale_fill_gradient(low = "yellow", high = "red")+ 
  facet_grid(tidy_coord_aprils$year ~ tidy_coord_aprils$category) + 
  labs ( x = "Drought Severity Index", y = "Year") + 
