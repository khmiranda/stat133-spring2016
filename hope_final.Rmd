...
drought_severity$year <- format(drought_severity$releaseDate, "%y")
...
tidy_severity <- drought_severity%>%
  ...
  filter(year == "00" | year == "05" | year == "10" | year == "11" | year == "12" | year == "13" | year == "14" | year == "15" | year =="16")%>%
...
CAcountiesvalues$month <- factor(CAcountiesvalues$month, levels = c("February", "May", "August", "November"))
...
#gonna try to make an even simpler thing to work with, gonna try it with nov2014 so it's a smaller dataset than say CAcountiesvalues

untidy <- tidy_severity%>%
  spread(key = category, value = ave_value)

f <- function(x) {
  ifelse(D4 >0 ,  select(subregion, month, year, D4),
         ifelse(D3 >0 ,  select(subregion, month, year, D3),
                ifelse(D2 >0 ,  select(subregion, month, year, D2),
                       ifelse(D1 >0 ,  select(subregion, month, year, D1),
                              ifelse(D0 >0 ,  select(subregion, month, year, D0), select(subregion, month, year, NONE))))))
}


f <- function(x) {
  ifelse(x[8] >0 ,  select(x[1], x[2], x[3], x[8]),
         ifelse(x[7] >0 ,  select(x[1], x[2], x[3], x[7]), select(x[1], x[2], x[3], x[6])
                )
          )
  }
```
