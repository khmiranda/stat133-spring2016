...
ggplot() +   
  geom_polygon(data = CAcountiesvalues, aes(x = long, y = lat, group = group), color = "grey80", fill = "grey80", size = .1) + 
  stat_density2d(data = CAcountiesvalues, aes(x = long, y = lat, color = category, fill = ..level.., alpha = ..level..), size = 1,  geom = "polygon") + scale_fill_gradient(low = "yellow", high = "red") + 
  facet_grid(month ~ year) + 
  labs(x = "Month", y = "year") +
    theme(axis.line=element_blank(),axis.text.x=element_blank(),axis.text.y=element_blank(),axis.ticks=element_blank(), legend.position="none")
```
